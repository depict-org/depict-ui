  ...
  return (
    <CommerceProvider locale={locale}>
      <DepictProvider
        merchant="stronger"
        market="gb"
        userId="next-commerce-user-1234"
        sessionId="next-commerce-session-1234"
        search={{
        ...
        displayTransformers={{
          categories: ({ data }) =>
            data.map((categoryDisplay: { listing_id: string }) => {
              const url_object = new URL(location.href)
              url_object.pathname = `/category/${categoryDisplay.listing_id}`
          ...
          products: ({ displays }) =>
            displays.map((display: any) => {
              if (!('variant_index' in display)) {
                ...
                return {
                  ...display,
                  variant_displays: display.variant_displays.map(
                    (variant_display: any) => ({
                      page_url: '/fake-url',
                      ...variant_display,
                    })
