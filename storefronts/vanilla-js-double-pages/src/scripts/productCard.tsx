...
export function productCard(display: StrongerDisplay | EquestrianDisplay | null) {
  ...
  const image_url = variant && "image_url" in variant ? variant.image_url : variant?.image_urls[0];
    ...
    <div class="product-card">
      <a href={`/pdp.html?product-id=${variant?.product_id}`}>
        ...
        <span class="text-container">
          ...
          <span class="price">{variant?.sale_price || <TextPlaceholder height="1em" width="4ch" />}</span>
        </span>
      </a>
    </div>
  );
