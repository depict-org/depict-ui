---
title: "Setting up JSX"
description: "How to set up JSX in your build environment to create HTMLElements"
---

To be able to easily create [HTMLElement](https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement)s in your code, we recommend setting up JSX in your build environment with our simple JSX runtime. JSX is a syntax extension to JavaScript that allows you to write HTML-like code in your JavaScript files. It is not required to use JSX, but it is in our opinion the best way to "template" our product data into the [DOM](https://developer.mozilla.org/en-US/docs/Web/API/Document_Object_Model/Introduction), since it has the same syntax as HTML but operates perfectly with JavaScript.
...
<CodeGroup>
    ...
    ```shell NPM
    npm install @depict-ai/utilishared @babel/plugin-transform-react-jsx
    ```

    ```shell Yarn
    yarn add @depict-ai/utilishared @babel/plugin-transform-react-jsx
    ```
</CodeGroup>
...
Create a file called `jsx-runtime.js` in your <i>project root</i> with the following contents:
...
<Accordion title="jsx.d.ts">
```ts jsx.d.ts
/**
 * Taken from solid-js and adapted. Solid in turn:
...
2. Update your tsconfig to include following:
...
```
`element` is now a [HTMLDivElement](https://developer.mozilla.org/en-US/docs/Web/API/HTMLDivElement) with two [children](https://developer.mozilla.org/en-US/docs/Web/API/Element/children): two HTMLDivElement's with in turn two [Text](https://developer.mozilla.org/en-US/docs/Web/API/Text)-nodes as [childNodes](https://developer.mozilla.org/en-US/docs/Web/API/Node/childNodes).
