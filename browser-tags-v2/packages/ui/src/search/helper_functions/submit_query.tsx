...
import { SearchFieldValue } from "./search_field_value";
 ...
 * @returns whether a navigation to the new query happened
 */
export async function submit_query_handler({
  ...
  if (previous_query === new_search_query && new URLSearchParams(location.search).has(search_param_name_)) {
    ...
    search_field_value_.settle_(new_search_query);
    on_submit_with_unchanged_value_?.();
    return false;
  }
  ...
  const end_submit = search_field_value_.begin_submit_(new_search_query, previous_query);
  before_navigate_?.();
  let landed: boolean;
  try {
